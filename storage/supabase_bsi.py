...
def upload_bsi_data():
    ...
    json_path = r'D:\exercises\Thesis\Json Files\bsi_standard.json'

    data = load_json(json_path)

    rows = transform_bsi_rows(data)
    ## Clear existing data before inserting new rows
    clear_table(table_name)
    ## Insert new rows
    insert_rows(table_name, rows)

    print(f"BSI data upload completed. Total rows inserted: {len(rows)}")
